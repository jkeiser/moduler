...
class Config
  ...
  def self.singleton
    @singleton ||= Chef::Config.new
  end
  Moduler::Scope.bring_into_scope(Chef::Config, singleton)

  Path = Moduler::Type.new(Moduler::Path, store_as: String)
  ...
  attribute :client_key, Path, relative_to: lazy { config_dir }, default: lazy { chef_zero.enabled ? nil : "client.pem" }

  # This secret is used to decrypt encrypted data bag items.
  attribute :encrypted_data_bag_secret, Path, relative_to: lazy { config_dir }, default: lazy do
    if File.exist?(Path.new(config_dir) + "encrypted_data_bag_secret")
  ...
  #
  # If chef-zero is enabled, this defaults to nil (no authentication).
  attribute :validation_key, Path, relative_to: lazy { config_dir }, default: lazy { chef_zero.enabled ? nil : "validation.pem" }
